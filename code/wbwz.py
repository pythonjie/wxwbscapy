...
def save_mysql(key,wburl):
    ...
    try:
        ...
        if int(dayss) < int(ts):
            ...
            try:
                sql = "REPLACE into v_xlwbwz (wbmc,wburl,bwnr,wzurl,zfs,dzs,pls,fbsj,zhfbsj,cjsj) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');"%(wbmc,wburl,str(bwnr),img_url,zfs,dzs,pls,fbsj,zhfbsj,cjsj);
                #print(sql)
                curos.execute(sql)
                db.commit()
                # #print(sql)
